...
public String fullOrderMail;

    public void getOrder(){
        fullOrderMail = mailGohan + mailSalad + mailDrinks + mailCombo1 + mailCombo2 + mailCombo3;

    }

    public void generateMail() throws InterruptedException{
        ...
        try{
            ...
            startActivity(Intent.createChooser(sendOrder, "Enviar correo"));
        }
    ...
    Runnable myRunnable = new Runnable() {
        ...
        volatile boolean isOrderMethodDone = false;
        boolean isMailMethodDone = false;

        @Override
        public void run() {

            if ( isOrderMethodDone == false ) {
                getOrder();
                isOrderMethodDone = true;
            }

            if(isOrderMethodDone){
                try {
                    generateMail();
                } catch (InterruptedException e) {
                    e.printStackTrace();
    ...
    void stopTask() {
        //Remove pending posts of Runnable that are in message queue
